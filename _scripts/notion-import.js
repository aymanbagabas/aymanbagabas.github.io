...
const modifyNumberedListObject = (
	blocks
) => {
		...
		for (const b of blocks) {
			if (b.type === 'image' && b.image.type === 'file') {
				...
				if (b.image.caption?.length > 0) {
					...
					if (caption) {
						fileName = caption.replace(/[^a-z0-9]/gi, '_').toLowerCase() + path.extname(fileName)
					}
				}
				let img = Buffer.alloc(0)
				await fetch(url).then(res => res.buffer()).then(buf => (img = buf))
